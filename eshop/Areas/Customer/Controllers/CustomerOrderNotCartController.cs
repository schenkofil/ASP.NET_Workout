...
namespace eshop.Areas.Customer.Controllers
{
    ...
    public class CustomerOrderNotCartController : Controller
    {
        ...
        public double AddOrderItemsToSession(int? productId)
        {
            ...
            {
                totalPrice = HttpContext.Session.GetInt(totalPriceString).GetValueOrDefault();
            }
            ...
            if (product != null)
            {
                ...
                if (HttpContext.Session.IsAvailable)
                {
                    ...
                    if (orderItemInSession != null)
                    {
                        ...
                        //cenova hladina -> pri koupi peti stejnych vyrobku je na kazdy kus sleva 500 kc
                        if (orderItemInSession.Amount >= 5)
                        {
                            orderItemInSession.Price = orderItem.Product.Price * orderItemInSession.Amount - (orderItemInSession.Amount * 500);
                            totalPrice = (totalPrice - (orderItem.Product.Price * orderItemInSession.Amount)) + (orderItem.Product.Price * orderItemInSession.Amount - (orderItemInSession.Amount * 500));
                        }
                        else
                        {
                    ...
                    HttpContext.Session.SetInt(totalPriceString, totalPrice);
                }
            }
        ...
        public async Task<IActionResult> ApproveOrderInSession()
        {
            if (HttpContext.Session.IsAvailable)
            {
                ...
                if (orderItems != null)
                {
                    ...
                    {
                        if (orderItem.Amount >= 5)
                        {
                            totalPrice = (orderItem.Product.Price - 500 * orderItem.Amount);
                        }
                        else
                        {
